...
> **驱动安装**：`yaspy` 已可通过 PyPI 安装（`pip install yaspy`）。本仓库 `setup.py` 已声明 SQLAlchemy 运行时依赖；`yaspy` 仍需运行应用前自行安装。开发/跑连库测试时，执行 `pip install -r requirements_dev.txt` 会一并安装 `yaspy`（文件中未钉死版本号时，由 pip 从当前索引解析并安装最新兼容版本）。
...
其中包含例如 `SQLAlchemy==2.0.50`、`pytest>=7.0.0,<10`、`yaspy` 等（详见 `requirements_dev.txt`；`yaspy` 未写版本号时由 pip 选取索引上的可用版本）。
...
| URL 前缀 | 说明 |
|----------|------|
| `yashandb+yaspy://...` | 使用 **yaspy** 驱动（**唯一对外推荐**） |
| `yashandb://...` | 默认映射到 **yaspy**（与 entry point `yashandb = ...yaspy` 一致） |

> **关于 yasdb**：历史方言 `yashandb+yasdb://...` 仍可能随旧版 entry point 存在，但**已不再维护、不对外推广、不纳入本分支测试**。新应用请统一使用 **yaspy**。
...
- 最低数据库版本为 **YashanDB 23.4.12.1**（不兼容 23.4.1.102 等更早版本）。
- 单行 **`UPDATE` … RETURNING** 在 23.4.12.1+ 上经 yaspy 支持；**`DELETE` … RETURNING** 仍不支持（`YAS-04209`）；executemany RETURNING 仍不适用。
- 部分与空字符串与 NULL 语义、极端标识符等相关的 SQLAlchemy 官方用例会被声明为不适用或跳过。
- 具体以当前代码与测试结论为准。
