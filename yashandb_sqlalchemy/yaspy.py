    ...
    def do_set_input_sizes(self, cursor, list_of_tuples, context):
        if self.positional:
            ...
            for i, parameter_row in enumerate(context.parameters):
                ...
                for j, ((key, dbtype, sqltypes), pval) in enumerate(zip(list_of_tuples, parameter_row)):
                    if dbtype in [self.dbapi.CHAR, self.dbapi.NCHAR]:
                        if i == 0:
                            replaced_parameter_row.append(cursor.var(self.dbapi.CHAR, arraysize=len(context.parameters)).setvalue(pval, i))
                        else:
                            replaced_parameter_row.append(replaced_parameters[-1][j].setvalue(pval, i))
                    else:
